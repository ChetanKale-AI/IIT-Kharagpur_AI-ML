...
class AlexNet(nn.Module):
    def __init__(self, num=10):
        ...
        # Define the feature extractor according to the problem statement
        self.feature = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=5, stride=1, padding=1),  # 32 filters, 5x5 kernel, padding=1
            nn.ReLU(inplace=True),
            nn.Conv2d(32, 64, kernel_size=3, stride=1),  # 64 filters, 3x3 kernel, no padding
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),  # MaxPool 2x2, stride=2
            nn.Conv2d(64, 96, kernel_size=3, stride=1),  # 96 filters, 3x3 kernel
            nn.ReLU(inplace=True),
            nn.Conv2d(96, 64, kernel_size=3, stride=1),  # 64 filters, 3x3 kernel
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 32, kernel_size=3, stride=1),  # 32 filters, 3x3 kernel
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=1)  # MaxPool 2x2, stride=1
        )

        # Define the classifier according to the problem statement
        self.classifier = nn.Sequential(
            nn.Linear(32 * 5 * 5, 2048),  # Adjust input size based on the actual output of the feature extractor
            nn.ReLU(inplace=True),
            nn.Dropout(),
            nn.Linear(2048, 1024),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        x = self.feature(x)
        print(x.shape)  # Print the shape after the feature extractor to check its size
        # Flatten the output
        x = x.view(x.size(0), -1)
